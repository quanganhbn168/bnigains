...
class SaveGainsProfileAction
{
    public function execute(array $rawData, bool $updateExisting = true, bool $importDriveImages = true): string
    {
        $data = $this->normalizeRow($rawData);

        // Nếu rỗng thông tin cơ bản thì báo bỏ qua
        if (blank($data['full_name']) && blank($data['email']) && blank($data['phone'])) {
            return 'skipped';
        }

        return DB::transaction(function () use ($data, $updateExisting, $importDriveImages) {
            $profile = $this->findExistingProfile($data);

            if ($profile && ! $updateExisting) {
                return 'duplicate';
            }
    ...
    // =========================================================================
    // TOÀN BỘ CÁC HÀM BÊN DƯỚI ĐƯỢC BÊ NGUYÊN XI TỪ IMPORT CỦA ANH SANG
    // =========================================================================

    protected function normalizeRow(array $row): array
    {
        return [
            'full_name' => $this->pick($row, ['ho_va_ten', 'họ_và_tên', 'ho_ten', 'họ_tên', 'full_name', 'name']),
            'bni_position' => $this->pick($row, ['chuc_vu_bni', 'chức_vụ_bni', 'chuc_vu_tai_bni', 'chức_vụ_tại_bni', 'bni_position']),
            'chapter_name' => $this->pick($row, ['chapter', 'ten_chapter', 'tên_chapter', 'chapter_name']) ?: 'BNI KINHBAC CHAPTER',
            'date_of_birth' => $this->normalizeDate($this->pick($row, ['ngay_sinh', 'ngày_sinh', 'ngay_thang_nam_sinh', 'ngày_tháng_năm_sinh', 'date_of_birth', 'birthday'])),
            'address' => $this->pick($row, ['dia_chi', 'địa_chỉ', 'address']),
            'phone' => $this->normalizePhone($this->pick($row, ['phone', 'so_dien_thoai', 'số_điện_thoại', 'dien_thoai', 'điện_thoại'])),
            'email' => strtolower(trim((string) $this->pick($row, ['email', 'email_ca_nhan', 'email_cá_nhân', 'email_cong_viec', 'email_công_việc', 'dia_chi_email', 'địa_chỉ_email']))),
            'education' => $this->pick($row, ['bang_cap', 'bằng_cấp', 'bang_capchung_chi', 'bang_cap_chung_chi', 'bằng_cấp_chứng_chỉ', 'hoc_van', 'học_vấn', 'education']),
            'qualifications' => $this->pick($row, ['bang_cap', 'bằng_cấp', 'bang_capchung_chi', 'bang_cap_chung_chi', 'bằng_cấp_chứng_chỉ', 'qualifications']),
            'personal_album_urls' => $this->extractUrls((string) $this->pick($row, ['album_ca_nhan', 'album_cá_nhân', 'anh_ca_nhan', 'ảnh_cá_nhân', 'personal_photos'])),
            'business_album_urls' => $this->extractUrls((string) $this->pick($row, ['album_doanh_nghiep', 'album_doanh_nghiệp', 'anh_doanh_nghiep', 'ảnh_doanh_nghiệp', 'business_photos'])),
            'product_album_urls' => $this->extractUrls((string) $this->pick($row, ['album_san_pham_dich_vu', 'album_san_phamdich_vu', 'album_sản_phẩm_dịch_vụ', 'album_san_pham', 'album_sản_phẩm', 'product_photos', 'product_gallery'])),
            'company_name' => $this->pick($row, ['cong_ty', 'công_ty', 'company', 'company_name', 'ten_cong_ty', 'tên_công_ty']),
            'job_title' => $this->pick($row, ['chuc_danh', 'chức_danh', 'chuc_danh_tai_doanh_nghiep', 'chức_danh_tại_doanh_nghiệp', 'job_title', 'vi_tri', 'vị_trí']),
            'business_category' => $this->pick($row, ['linh_vuc', 'lĩnh_vực', 'business_category', 'nganh_nghe', 'ngành_nghề']),
            'core_products' => $this->pick($row, ['san_pham_chinh', 'sản_phẩm_chính', 'core_products', 'san_pham', 'sản_phẩm']),
            'accompanying_services' => $this->pick($row, ['dich_vu_di_kem', 'dịch_vụ_đi_kèm', 'accompanying_services']),
            'highlight_products' => $this->pick($row, ['san_pham_noi_bat', 'sản_phẩm_nổi_bật', 'highlight_products']),
            'g_goals' => $this->pick($row, ['goals', 'goals_muc_tieu', 'goals_mục_tiêu', 'g_goals', 'muc_tieu', 'mục_tiêu']),
            'a_accomplishments' => $this->pick($row, ['accomplishments', 'accomplishments_thanh_tuu', 'accomplishments_thành_tựu', 'a_accomplishments', 'thanh_tuu', 'thành_tựu']),
            'i_interests' => $this->pick($row, ['interests', 'interests_so_thich', 'interests_sở_thích', 'i_interests', 'so_thich', 'sở_thích']),
            'n_networks' => $this->pick($row, ['networks', 'networks_mang_luoi_quan_he', 'networks_mạng_lưới_quan_hệ', 'networks_mang_luoi_ket_noi', 'networks_mạng_lưới_kết_nối', 'n_networks', 'mang_luoi_quan_he', 'mạng_lưới_quan_hệ']),
            's_skills' => $this->pick($row, ['skills', 'skills_ky_nang', 'skills_kỹ_năng', 's_skills', 'ky_nang', 'kỹ_năng']),
        ];
    }
    ...
    protected function importDriveImages(GainsProfile $profile, array $urls, string $collectionName): void
    {
        ...
        foreach ($urls as $url) {
            ...
            try {
                ...
                // Đổi thành ghi log thay vì ghi vào $this->errors để Action độc lập với luồng Excel
                Log::warning('Không import được ảnh vào ' . $collectionName . ': ' . $url . ' - ' . $e->getMessage());
            } finally {
